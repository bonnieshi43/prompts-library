...
| **TC-N01** | 负向用例 | Where can I use CALC.sumif to calculate total sales? | `false` | CALC 公式函数（GUI 辅助，非脚本） | CALC 函数（例如 `CALC.sumif`）通过 Formula Editor 对话框输入 — 这是一个 GUI 辅助功能，不需要 JavaScript 编写。虽然它们类似于函数调用，但它们不是脚本，不得触发 `script=true`。 |
...
| **TC-M02** | 多轮对话 | **Previous**: How do I add a filter condition to a table using the filter toolbar? (script=`false`) <br> **Current**: Can I achieve the same filtering logic by writing a JavaScript expression instead? | `true` | 非脚本历史，当前轮引入脚本意图 | 前一轮是非脚本 UI 问题。当前轮独立引入脚本触发内容：编程语言引用（JavaScript）和明确的表达式编写意图。当前查询自己的信号必须占主导地位。验证单轮可以在不被非脚本历史抑制的情况下将 `script` 从 `false` 切换到 `true`。 |
| **TC-M03** | 多轮对话 | **Previous**: How do I write a script to dynamically set the foreground color of a Text component? (script=`true`) <br> **Current**: How do I change the font size for the same component using the Format panel? | `false` | 脚本历史，当前轮无关的 UI 操作 | 前一轮是面向脚本的，但当前轮是一个独立的 UI 配置问题（Format 面板），没有编码意图。必须独立评估当前查询并产生 `script=false`。验证当当前查询明确是非脚本时，先前的脚本上下文不会"渗透"到当前轮。 |",
    "new_string": "| **TC-G01** | gui_required 覆盖 | How do I open the Script tab in the component properties panel and write a JavaScript expression to hide a text label? | `false` | gui_required 覆盖 script（isScriptModule 逻辑） | 查询包含两个脚本触发信号：命名 JavaScript（规则 2）和提到编写表达式（规则 1）。但是，它还需要导航 GUI（在属性面板中打开 Script 选项卡），这设置了 `gui_required=true`。`isScriptModule(gui_required=true, script=true)` 函数必须返回 `false`，因为 `gui_required` 优先。验证在 LLM 产生 `script=true` 后正确应用代码级覆盖逻辑。 |
| **TC-G02** | gui_required 覆盖 | How do I navigate to the Script Editor panel and write an expression to dynamically change the chart title? | `false` | GUI 导航 + 脚本编写混合，gui_required 覆盖 script（isScriptModule 逻辑） | 与 TC-G01 意图相同但措辞不同，用于验证 gui_required 判定的语言稳定性。"navigate to" 明确是 GUI 操作步骤，即使后半句提到编写表达式，`gui_required=true` 必须优先，`isScriptModule(true, true) = false`。验证：不同措辞的 GUI 导航请求均能稳定触发 `gui_required=true`。 |
| **TC-G04** | gui_required 覆盖 | I want to write a JavaScript expression for a component — which tab do I need to open first in the properties panel? | `false` | 编程语言引用（规则 2）+ GUI 步骤引导请求，gui_required 覆盖 script | 查询包含 JavaScript（触发规则 2，script=true），同时用户要求被引导完成"先打开哪个 tab"，表明需要 GUI 操作步骤，应触发 `gui_required=true`。`isScriptModule(true, true)` 必须返回 `false`。验证：规则 2 与 gui_required 覆盖同时触发时，代码层覆盖逻辑正确运行。 |
| **TC-M01** | 多轮对话 | **Previous**: How do I write a script in the onInit handler to load data using runQuery and assign the first record's value to a Text component? (script=`true`) <br> **Current**: Yes, that approach looks right. How do I properly structure the variable assignment? | `true` | 脚本导向的前一轮的确认 | 当前一轮建立了脚本上下文（明确的脚本编写意图）时，当前轮中的简短确认或后续问题不包含独立的脚本触发器。`completeQueryByHistory` prompt 必须检测历史中的 `script=true` 并重建面向脚本的 `replaced_query`（例如，"如何在使用 runQuery 结果的 onInit 脚本中构造变量赋值？"）。然后 `querySubjectAreas` prompt 评估此重写的查询并必须产生 `script=true`。验证历史驱动的查询补全保留脚本方向。 |
| **TC-M02** | 多轮对话 | **Previous**: How do I add a filter condition to a table using the filter toolbar? (script=`false`) <br> **Current**: Can I achieve the same filtering logic by writing a JavaScript expression instead? | `true` | 非脚本历史，当前轮引入脚本意图 | 前一轮是非脚本 UI 问题。当前轮独立引入脚本触发内容：编程语言引用（JavaScript）和明确的表达式编写意图。当前查询自己的信号必须占主导地位。验证单轮可以在不被非脚本历史抑制的情况下将 `script` 从 `false` 切换到 `true`。 |
| **TC-M03** | 多轮对话 | **Previous**: How do I write a script to dynamically set the foreground color of a Text component? (script=`true`) <br> **Current**: How do I change the font size for the same component using the Format panel? | `false` | 脚本历史，当前轮无关的 UI 操作 | 前一轮是面向脚本的，但当前轮是一个独立的 UI 配置问题（Format 面板），没有编码意图。必须独立评估当前查询并产生 `script=false`。验证当当前查询明确是非脚本时，先前的脚本上下文不会"渗透"到当前轮。 |
| **TC-M04** | 多轮对话 | **Previous Turn 1**: How do I write a script in the onInit handler to load data using runQuery and assign the first record's value to a Text component? (script=`true`) <br> **Previous Turn 2**: How do I change the font size for the Text component using the Format panel? (script=`false`) <br> **Current**: Can I use runQuery in the same onInit handler to load a second dataset? | `true` | 脚本历史中间插入非脚本轮后，继续脚本话题 | 对话历史中最近一轮是非脚本的 UI 问题（TC-M03 模式），但当前轮重新引入明确的脚本意图（调用 runQuery）。`completeQueryByHistory` 使用 Stack 从最新一轮向前搜索，应识别当前轮自身携带的脚本信号，而非被上一轮的非脚本历史压制。验证：非脚本历史不会阻止当前轮独立判定为 `script=true`。 |
| **TC-M05** | 多轮对话 | **Previous**: How do I write a script to calculate the aggregation of a field and assign the result to a label? (script=`true`) <br> **Current**: Sales Amount | `true` | 脚本历史中，用户以字段名作为选项确认，script 方向应保留 | `completeQueryByHistory` prompt 的 Selection 处理规则规定，当用户回复是一个选项/字段名时，需要结合历史重建高层意图，并在历史 `script=true` 时保持脚本导向。用户回复 "Sales Amount" 本身不含脚本信号，但历史上下文是脚本操作，`replaced_query` 必须被重建为表达式/脚本导向（例如"如何在表达式中引用 Sales Amount 字段"）。验证：字段名确认不会将脚本历史上下文切换为非脚本。 |
| **TC-F01** | 负向用例（边界） | What built-in functions are supported in the StyleBI Formula Editor? | `false` | "function" 指内置函数文档查询，非代码调用（TC-P07 的边界） | 用户在询问产品中有哪些内置函数可用，是文档检索性质的提问。"function" 出现了但用户意图是查阅功能列表，而非编写/调用代码。规则 7 要求 "function-based operations"（使用/调用函数），纯粹询问函数列表不满足该条件，必须产生 `script=false`。 |
| **TC-F03** | 负向用例（边界） | What is the difference between the NOW() function and the TODAY() function in StyleBI? | `false` | "function" 出现在概念比较问题中，非调用意图（TC-P07 的边界） | 用户对两个内置函数进行概念性比较，没有编写、调用或操作任何函数的意图。"function" 出现多次但全部是名词性引用，无调用语义。必须产生 `script=false`。 |
| **TC-E03** | 正向用例（边界） | How do I use the field["fieldName"] syntax to reference a column value? | `true` | field["fieldName"] 语法引用，脚本专属表达式语法 | `field["fieldName"]` 是 StyleBI 脚本/表达式中的字段引用语法，只在脚本上下文中出现，`completeQueryByHistory` 的 Script Identification Rule 也专门列举了此语法。用户询问该语法的使用方式，意图明确是理解/编写脚本表达式。必须产生 `script=true`。 |
