...
**两者是否一致？** URL 是否正确、错误分支是否都处理、副作用是否在正确时机触发、**错误分支的返回值是否适合生产环境**（是否混入了 mock / 开发用 fallback 数据）。
...
| 等级 | 含义 | 是否生成 | 每 group 上限 |
|---|---|---|---|
| **Risk 3** | 静默失败、错误分支、多条件分支的关键路径、数据损坏、安全相关 | 必须 | ≤ 4 |
| **Risk 2** | happy path、URL/请求体正确性、成功后副作用、重要契约 | 每个场景至少 1 条 | ≤ 3 |
| **Risk 1** | 与已有 case 高度相似的参数变体、可从 happy path 直接推导的情况 | 跳过 | — |

> 超出上限时，**保留触发路径最独立的 case**，其余降为 Risk 1 跳过。
...
**生成规则：**
- 一个场景方法对应**一个 `describe`**，按 Risk 3 → Risk 2 顺序排列，遵守上表每 group 上限
...
| 状态对称性 | 合并 / 优先级函数：每个状态值是否在两侧（old / new）被一致保护？ |

同样标注 `[Risk N]`，Risk 1 跳过。同类边界合并进同一个 `it`。

**生成规则：** 一个纯逻辑方法对应**一个 `describe`**，按 Risk 3 → Risk 2 顺序排列，遵守上表每 group 上限。
